...
class ForwardS(ArrayStepShared):
    ...
    def __init__(self, vars, nObs, T, N, observed_jumps, model=None):
        ...
        # print self.shared
        pi = stick_breaking.backward(self.shared['pi_stickbreaking_'])
        lower = model.free_RVs[1].distribution.dist.lower
        upper = model.free_RVs[1].distribution.dist.upper
        Q = rate_matrix_one_way(lower, upper).backward(self.shared['Q_ratematrixoneway_'])
    ...
    def computeBeta(self, Q, B, likelihoodOfS):
        ...
        for n in xrange(self.N):
            ...
            for t in np.arange(T[n] - 1, 0, -1):
                ...
                beta[n0 + t - 1, :] = np.sum(
                    beta[n0 + t, np.newaxis, :] * pS[tau_ind, :, :] * likelihoodOfS[n0 + t, :, :], axis=1)
    ...
    def computeLikelihoodOfS(self, X, B, B0):
        ...
        probTemp = ((1 - commorb_was_off) * (1 - commorb_changed))[:, :, np.newaxis] + commorb_was_off[:, :,
                                                                                       np.newaxis] * (
                                                                                       commorb_changed[:, :,
        ...
        # Set t=0
        likelihoodOfS[self.zeroIndices, :, :] = (X[self.zeroIndices][:, :, np.newaxis] * B0[np.newaxis, :, :] + (1 - X[
            self.zeroIndices])[:, :, np.newaxis] * (1 - B0)[np.newaxis, :, :]).prod(axis=1)[:, np.newaxis, :]
